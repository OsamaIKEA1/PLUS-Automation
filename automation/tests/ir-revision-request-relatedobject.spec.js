...
test.beforeAll(async ({ browser, playwright }) => {
  ...
  columnsNamesNewIrPageSupplier = await supplierHelper.saveColumnsNamesNewIrPageRelatedObject(newPage);
//   columnsNamesOngoingPageSupplier = await supplierHelper.saveColumnsNamesOngoingPageRelatedObject(newPage);
//   columnsNamesRevisionRequestPageSupplier = await supplierHelper.saveColumnsNamesRevisionRequestPageRelatedObject(newPage);
...
//   columnsNamesRevisionRequestPagePurchasing = await purchasingHelper.saveColumnsNamesIrTableRevisionRequestPageRelatedObject(newPage2);
...
test.describe("Raise Revision Request on IR by selecting single or multiple IR's from List Page or Details Page", () => {
  ...
  test("Test Case #1: Testing IR from Details Page from the New IR page with different request implementation dates and send button behavior", async ({ playwright }) => {
    ...
    await test.step("Select the IR Details from New IR and provide new suggested request impl.date prior to current date", async () => {
      ...
      await supplierHelper.navigateToNewIrPageRelatedObject(sharedData.page);
    //   await supplierHelper.searchForIrWithArticleNumber(sharedData.page, dynamicValues.testEnvObjectNumber);
      ...
      /**
       * Before clicking on the Object name column, we need to check if the IR is marked as Strict or not (Under column: "Req.Impl.Date").
       * If the IR is Strict, then we cannot raise a revision request, so we need to check the next row until we have gone through all the rows in the table.
       */

      var columnStrict = await locators.irColumnForFirstRow(sharedData.page).nth(columnsNamesNewIrPageSupplier.get(dynamicValues.ReqestedImplementationDate));
      let isStrict = await supplierHelper.isDateStrict(columnStrict);

      // Loop through the rows until we find a non-strict IR or we reach the end of the table
      let rowIndex = 0;
      const totalRows = await locators.irRows(sharedData.page).count();

    while (isStrict && rowIndex < totalRows) {
      // Get the row element
      const rowElement = await locators.irColumn(sharedData.page, rowIndex);
    
      // Scroll the row into view before trying to access it
      await rowElement.scrollIntoViewIfNeeded();
    
      // Now get the date column and check if strict
      columnStrict = await rowElement.nth(columnsNamesNewIrPageSupplier.get(dynamicValues.ReqestedImplementationDate));
      isStrict = await supplierHelper.isDateStrict(columnStrict);
    
      rowIndex++;
      await sharedData.page.waitForTimeout(200);
    }

// If we exited because we ran out of rows
if (rowIndex >= totalRows) {
    throw new Error("Could not find a non-strict IR in the table");
}

      // Now select the Object name column for the non-strict IR
      var columnObjectName = await locators.irColumn(sharedData.page, rowIndex).nth(columnsNamesNewIrPageSupplier.get(dynamicValues.ObjectName));
      await sharedHelper.ensureVisible(sharedData.page, columnObjectName);
      await columnObjectName.locator('ptcs-link').click();
